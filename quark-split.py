...
from utils import custom_print, get_datetime, get_timestamp
...
class QuarkPanFileManager:
            ...
            while True:
                ...
                for file in _list:
                    d: Dict[str, Union[int, str]] = {
                        ...
                        "include_items": (
                            file["include_items"] if "include_items" in file else ""
                        ),
    ...
    async def create_dir(self, pdir_name="新建文件夹", pdir_fid="0") -> None:
        params = {
            "pr": "ucpro",
            "fr": "pc",
            "uc_param_str": "",
        ...
        if data_list:
            if download:
                for i in data_list:
                    if i["dir"]:
                        custom_print(
                            f'开始下载：{i["file_name"]} 文件夹中的{i["include_items"]}个文件'
                        )
                        _, data_list = await self.get_detail(
                            pwd_id, stoken, pdir_fid=i["fid"]
                        )
                        folder = i["file_name"]
                    else:
                ...
                for f in data_list:
                    f["full_name"] = f["file_name"]
                files_count = 0
                folders_count = 0
                level = 0
                while level < directoryLevel:
                    level += 1
                    save_file_list = [d for d in data_list]
                    data_list.clear()
                    for save_file in save_file_list:
                        full_name = save_file["full_name"]
                        if save_file["dir"]:
                            if (
                                "/每日更新最新短剧/2023年月份合集/7月更新/".find(
                                    f'/{save_file["file_name"]}/'
                                )
                                > -1
                            ):
                                # 暂停一段时间
                                await asyncio.sleep(random.randint(500, 1000) / 1000)
                                _, child_list = await self.get_detail(
                                    pwd_id, stoken, save_file["fid"]
                                )
                                custom_print(
                                    f"文件夹：{full_name}，文件数：{len(child_list)}"
                                )
                                for child in child_list:
                                    child["full_name"] = (
                                        f'{full_name}/{child["file_name"]}'
                                    )
                                data_list.extend(child_list)
                        else:
                            data_list.append(save_file)
                            files_count += 1
                ...
                for data in data_list:
                    if data["dir"]:
                        ...
                        files_count += 1
                custom_print(
                    f"转存总数：{total_files_count}，文件数：{files_count}，文件夹数：{folders_count}"
                )

                data_list = [
                    data_list[i : i + 100] for i in range(0, len(data_list), 100)
                ]
                for data in data_list:
                    fids = [d["fid"] for d in data]
                    share_fid_tokens = [d["share_fid_token"] for d in data]
                    ...
                    await asyncio.sleep(1)  # 暂停一段时间，太快了会直接提示转存失败
            print()
    ...
    async def get_share_save_task_id(
        ...
        async with httpx.AsyncClient() as client:
            ...
            if json_data["status"] != 200:
                custom_print(f"转存失败：{json_data['message']}")
                return None
            task_id = json_data["data"]["task_id"]
            tq_gap = json_data["metadata"]["tq_gap"]
            custom_print(f"获取任务ID：{task_id} 文件数量：{tq_gap}")
            return task_id
            ...
            if json_data["status"] == 200:
                if json_data["data"]["status"] == 2:
                    ...
                    if json_data["data"]["task_title"] == "分享-转存":
                        tq_gap = json_data["metadata"]["tq_gap"]
                        custom_print(f"结束任务ID：{task_id} 文件数量：{tq_gap}")
                ...
                if (
                    ...
                    input(
                        f"[{get_datetime()}] 转存失败，网盘容量不足！请注意当前已成功保存的个数，避免重复保存"
                    )
                elif json_data["code"] == 41013:
                    input(
                        f"[{get_datetime()}] “{to_dir_name}” 网盘文件夹不存在，请重新运行按3切换保存目录后重试！"
                    )
                else:
...
def print_menu() -> None:
    print(
        """
╔══════════════════════════════════════════════════════════════════════════════════════════════════════════╗
║                                  Author: sudot  Version: 0.0.3                                           ║
║                          GitHub: https://github.com/sudot/QuarkPanTool                                   ║
╠══════════════════════════════════════════════════════════════════════════════════════════════════════════╣
║     1.单个分享地址转存   2.批量分享地址转存   3.切换网盘保存目录   4.创建网盘文件夹   5.下载到本地   q.退出     ║
╚══════════════════════════════════════════════════════════════════════════════════════════════════════════╝
"""
    )
            ...
            if input_text.strip() == "1":
                ...
                if url and len(url.strip()) > 20:
                    ...
                    try:
                        directoryLevel = int(
                            input(
                                "请输入遍历层级(大于0的数字)（解决文件夹数量过多无法转存的问题）："
                            )
                        )
                    except:
                        pass
                    asyncio.run(
                        quark_file_manager.run(
                            url.strip(), to_dir_id, False, directoryLevel
                        )
                    )
                ...
                if create_name:
                    asyncio.run(
                        quark_file_manager.create_dir(pdir_name=create_name.strip())
                    )
                else:
